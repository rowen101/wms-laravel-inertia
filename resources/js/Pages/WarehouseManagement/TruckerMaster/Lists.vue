...
const userHeader = [
  ...
  {label: 'Trucker Name', fieldName: 'name'},
...
const deleteRecord =  (id) =>{
   axios.post('/wm/trucker/delete/'+id)
    .then(response => {
      toast.success(`Trucker record successfully deleted!`);
...
<Head title="truckers" />
  <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Truckers" main>
    <BaseButton
  ...
  <CardBox class="flex-1 p-6" has-table>
    <CoreTable v-if="tableRows.length > 0"
                ...
                @openLink="openLink"
    />
    <CardBoxComponentEmpty v-else >Click Add Trucker button to create your first record</CardBoxComponentEmpty>
  </CardBox>
  <AsideDrawer title="Trucker Info" :is-open="openDrawer" @closeDrawer="openDrawer = false" class="shadow-lg shadow-blue-500/50">

    <FormField label="Trucker Code">
      <FormControl v-model="formdata.code" name="code" :disabled="true" />
    </FormField>
    <FormField label="Trucker Name">
  ...
  <PopupDialog
    ...
    theme="danger"
    @popaction="popAction"> 
    <p class="text-sm text-gray-500">{{ 'Are you sure you want to delete trucker record ' + (formdata.code +', ' + formdata.name).toLocaleUpperCase() +' ? This action cannot be undone and account will be permanently removed.' }}</p>
  </PopupDialog>
</template>
